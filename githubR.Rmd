...
Here is what a user account looks like, with a list of repositaries (repo) and a heatmap of contributions. A repositary is like a folder where files are stored and organized. This folder can be synchronized with a folder on your local computer.
...
Now that our *README.md* file has been commited twice, we have snapshots of the file at two different points. These commits are saved on your local computer, but they are not yet **pushed** on your online GitHub repo. To do this, we need to push the different commits to the repositary. This can be done using the green arrow in the *Git* tab or in the commit window (so, just after a commit). This will update the online repo with the latest changes made to your local computer. 
...
Another and probably easier way is to first create a GitHub online repositary and then to clone this repositary using a version controled R Git project.
